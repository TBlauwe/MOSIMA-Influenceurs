...
to compute-influence
  ...
  ;; ===== BFS =====
  ;; Get all followers
  let fifo (sort in-link-neighbors)
  let followers (list self)

  while [length fifo > 0][

    let _user (first fifo)
    set fifo (but-first fifo)
    set followers (lput _user followers )

    foreach (sort [in-link-neighbors] of _user)[ child ->
      if(not member? child followers)[
        set fifo (lput child fifo )
      ]
    ]

  ]
  set influence (length followers)
end

to update-appearance
  set color influence-color influence
  set size influence-size influence
end
...
to-report get-max-influence
  report max [influence] of users
end

to-report get-min-influence
  report min [influence] of users
end
...
to-report influence-color [_value]
  report palette:scale-gradient (list (extract-rgb Color-Low) (extract-rgb Color-High)) _value get-min-influence get-max-influence
end

to-report influence-size [_value]
  if (_value = 0)[
    report Size-Low
  ]
  report lerp Size-Low Size-High (normalize get-min-influence get-max-influence _value)
end
...
to-report lerp [_min _max _value]
    report _min + _value * (_max - _min);
end
